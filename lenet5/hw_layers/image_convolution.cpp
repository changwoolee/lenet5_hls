...
#pragma HLS array_partition variable=input block factor=32 dim=3
...
#pragma HLS unroll
						for(int j=0;j<CONV_1_WH;j++){
#pragma HLS unroll
							mult[i*5+j] = input[batch_cnt][row+i][col+j]*kernel[depth_out][i][j];
						}
					}
...
*/
	...
	float input[CONV_1_TYPE][CONV_2_INPUT_WH][CONV_2_INPUT_WH];
	float kernel[CONV_2_TYPE][CONV_2_WH][CONV_2_WH];
	float bias[CONV_2_TYPE];
	float _temp[CONV_1_TYPE];
	float __temp[CONV_2_WH];

//#pragma HLS array_partition variable=bias complete dim=0
//#pragma HLS array_partition variable=input dim=1
//#pragma HLS array_partition variable=kernel dim=1
//#pragma HLS array_partition variable=_temp complete dim=0
//#pragma HLS array_partition variable=__temp complete dim=0
	int col, row;
	int col_f, row_f;
...
//#pragma HLS unroll
	...
	BATCH : for (batch_idx = 0; batch_idx < image_Batch; batch_idx++) {
		F_IN_J : for(int j=0;j<CONV_1_TYPE;j++){
			F_IN_K : for(int k=0;k<CONV_2_INPUT_WH;k++){
				F_IN_L : for(int l=0;l<CONV_2_INPUT_WH;l++){

					input[j][k][l] = input_feature[batch_idx*CONV_1_TYPE*CONV_2_INPUT_WH*CONV_2_INPUT_WH
		...
		D_OUT : for (depth_out = 0; depth_out < CONV_2_TYPE; depth_out++) {
			F_K_J : for(int j=0;j<CONV_1_TYPE;j++){
					F_K_K : for(int k=0;k<CONV_2_WH;k++){
							F_K_L : for(int l=0;l<CONV_2_WH;l++){
//#pragma HLS unroll
								kernel[j][k][l] = conv_kernel[depth_out*CONV_1_TYPE*CONV_2_WH*CONV_2_WH
			...
			ROW : for (row = 0; row < CONV_2_OUTPUT_WH; row++) {
				COL : for (col = 0; col < CONV_2_OUTPUT_WH; col++) {

					// Init
					temp = 0;
					float _temp[CONV_1_TYPE];
					for(int t=0;t<CONV_1_TYPE;t++)
						_temp[t] = 0;

					for(int t=0;t<CONV_2_WH;t++)
						__temp[t] = 0;
					// Multiplication by Convolution and Input feature maps
					D_IN : for (depth_in = 0; depth_in < CONV_1_TYPE; depth_in++) {
////#pragma HLS pipeline II=4

						//printf("temp is %.6f\n", temp);
						ROW_F : for (row_f = 0; row_f < CONV_2_WH; row_f++) {



							__temp[row_f]=((input[depth_in][row+row_f][0] * kernel[depth_in][row_f][0]
									+input[depth_in][row+row_f][1] * kernel[depth_in][row_f][1])
									 +(input[depth_in][row+row_f][2] * kernel[depth_in][row_f][2]
								  +input[depth_in][row+row_f][3] * kernel[depth_in][row_f][3]))
									 +input[depth_in][row+row_f][4] * kernel[depth_in][row_f][4];
							if(row_f==CONV_2_WH-1){
								_temp[depth_in]=((__temp[0]+__temp[1])+(__temp[2]+__temp[3]))+__temp[4];
								if(depth_in==CONV_1_TYPE-1){
									temp = ((_temp[0]+_temp[1])+(_temp[2]+_temp[3]))+(_temp[4]+_temp[5]);
									// Result of Convolution
									output_feature[(batch_idx * CONV_2_TYPE + depth_out)*CONV_2_OUTPUT_WH*CONV_2_OUTPUT_WH+
												   CONV_2_OUTPUT_WH * row + col] =(temp+bias[depth_out]);
								}
							}
						}
						//printf("\n");
